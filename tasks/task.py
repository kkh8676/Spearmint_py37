...
class Task(object):
    ...
    def numComplete(self, jobs):
        ...
        return len(list(filter(lambda job: self.name in job['tasks'] and job['status']=='complete', jobs)))

    def numPending(self, jobs):
        if not jobs:
            return 0

        return len(list(filter(lambda job: self.name in job['tasks'] and job['status']=='pending', jobs)))
